...
A Unity package that puts a fake audience in your scene. They clap, laugh, nod, and cheer — or
yawn and stop listening if you lose them.
...
**Step 1 — set the energy every frame.** 0 to 1. How much the speaker is giving the room.
...
Use this when the reaction comes from your code, not the speaker — a game event, a lesson script,
a network message. It ignores the room's mood.
...
```csharp
audience.Room.MoodLabel        // Cold → Electric
...
2. Has an `Animator`? The package fires a trigger named after each reaction. Add clips for the
   ones you want.
3. No `Animator`? Add `AudienceMemberView`, point it at the head and arm bones. Built-in
...
Not in WebGL — Unity has no microphone there. Set energy yourself instead.
...
`Runtime/Core` and `Runtime/Simulation` are plain C#, no Unity. Make an `AudienceRoom` and call
...
**Contagion.** Someone reacts → they nudge the people near them → those people are more likely to
react → they nudge their neighbours. So applause spreads out from one person, instead of everyone
rolling dice alone.

**Two moods.** Engagement (are they listening?) and warmth (do they like you?) are separate,
because they're different rooms — bored but friendly, or attentive but cold. Engagement moves
fast and drops as soon as energy drops. Warmth moves slowly and takes minutes to earn.
